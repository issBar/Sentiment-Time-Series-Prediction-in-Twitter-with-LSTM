...
def readFile(inquery):
    file_to_read="./csvData/predict_"+inquery+"_hashtag_tweets.csv"
    return file_to_read
...
model = Sequential()
model.add(LSTM(256,activation='tanh',input_shape=(n_steps, n_features),return_sequences=False))
#model.add(LSTM(56, activation='tanh'))
model.add(Dense(1, activation='tanh'))
model.summary()
model.compile(optimizer='adam', loss='mse')
early_stop = EarlyStopping(monitor='loss', patience=2, verbose=2)
history=model.fit(X, test, epochs=300,validation_split=0.1, shuffle=False,callbacks=[early_stop],verbose=1)
...
#results = r2_score(real_value, prediction)
#print("[==================================]\n\nAccuracy = ",results)
